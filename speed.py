...
class Game:
    ...
    def _get_inputs(self):
        ...
        # TODO: If the letter is a '*' clear the buffer. If not, add it.
        if letter == "*":
            self._buffer.clear()
        else:
    ...
    def _update_words(self):
        ...
        for word in self._words.get_all():
            word.move_next()
            if word.is_expired():
    ...
    def _do_outputs(self):
        ...
        actors = [self._buffer, self._score]
        actors.extend(self._words.get_all())
...
#   
#   self.get_position() <= MAX_X - (len(word) - 1)
class Word(Actor):
